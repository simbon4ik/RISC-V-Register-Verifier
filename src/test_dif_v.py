...
def scan_register_read_after_diff_write(register_space_start, register_space_end):
    """
    Function for check correct read after diff write with overflow
    """
    info_of_bug = []
        ...
        for value in range(MIN_VALUE, MAX_VALUE):
            ...
            if (reg.ack and reg.reg_value != value):
                info_of_bug.append({
                    ...
                    "description": (
                         "After writing reg doesn't "
                         "consist correct value"
                    ),
                    "FSM": [
                        "Starting Read after Different Write test",
                        "WRITE",
                        "READ",
                        "INCORRECT READ AND WRITE"
                    ]
                    })
                break
    return info_of_bug
